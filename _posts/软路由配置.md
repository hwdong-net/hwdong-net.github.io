...
规则中局域网的ip段（192.168.0.0）和v2ray监听的端口（12345）请结合实际情况修改。
...
iptables -t nat -A V2RAY -s 192.168.0.0/24 -p tcp -j REDIRECT --to-ports 12345
...
iptables -t mangle -A V2RAY -d 192.168.0.0/24 -j RETURN
# From lans redirect to Dokodemo-door's local port
iptables -t mangle -A V2RAY -p tcp -s 192.168.0.0/24 -j TPROXY --on-port 12345 --tproxy-mark 1
iptables -t mangle -A V2RAY -p udp -s 192.168.0.0/24 -j TPROXY --on-port 12345 --tproxy-mark 1
