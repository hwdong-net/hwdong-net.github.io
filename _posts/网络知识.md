...
bridge和switch是工作于链路层的连接设备，根据目标物理地**转发和过滤**（**forward and filtering**）数据包（packet），将输入的链路数据包转发到对应的出口。它们称为**full fledged packet switch**（全功能分组交换器）。bridge和switch的区别是switch的出口数量比较多。每个Bridge或switch里有一个表（Table）记录：lay-2地址(物理地址)和对应的端口以及更新的时间。  如果一个接口x接收了一个数据包，根据其中的目标地址找到对应的出口y，如果x==y就丢弃它（不转发），如果x!=y,就将它转发到y接口。 
